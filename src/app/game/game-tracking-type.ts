	...
	public cityVictory(): boolean {
		...
		GamePlayer.fromPlayer.forEach((gPlayer) => {
			...
			if (Alliances.teamGame) {
				if (
					Alliances.getInstance().getTeamCities(Alliances.getInstance().getPlayerTeam(gPlayer.player)) >=
					this._citiesToWin
				) {
		...
		GamePlayer.fromPlayer.forEach((gPlayer) => {
			...
			if (RoundSettings.diplomancy > 0 && Scoreboard.getInstance().allyBoard) {
				DisplayTimedTextToPlayer(
					...
					0.81,
					180.0,
					`             ${HexColors.WHITE}Team ${Alliances.getInstance().getPlayerTeam(who.player)}|r ${
						HexColors.TANGERINE
					}is ${PLAYER_COLOR_CODES[who.names.colorIndex]}victorious|r${HexColors.TANGERINE}!|r`
				);
				DisplayTimedTextToPlayer(
					gPlayer.player,
					0.73,
					...
					`${HexColors.WHITE}Team ${Alliances.getInstance().getPlayerTeam(who.player)}|r ${
						HexColors.TANGERINE
					}won the game with|r ${Alliances.getInstance().getTeamCities(
						Alliances.getInstance().getPlayerTeam(who.player)
				...
				DisplayTimedTextToPlayer(
					...
					0.81,
					180.0,
					`             ${PLAYER_COLOR_CODES[who.names.colorIndex]}${who.names.acct}|r ${HexColors.TANGERINE}is ${
						PLAYER_COLOR_CODES[who.names.colorIndex]
					}victorious|r${HexColors.TANGERINE}!|r`
				);
				DisplayTimedTextToPlayer(
					gPlayer.player,
					0.73,
					0.81,
					180.0,
					`${PLAYER_COLOR_CODES[who.names.colorIndex]}${who.names.acct}|r ${HexColors.TANGERINE}won the game with|r ${
						PLAYER_COLOR_CODES[who.names.colorIndex]
					}${who.cities.length}|r ${HexColors.TANGERINE}cities!|r`
				);
			}
			DisplayTimedTextToPlayer(
				gPlayer.player,
				...
				0.81,
				180.0,
				`             ${HexColors.TANGERINE}Discord: discord.me/risk|r`
			);
