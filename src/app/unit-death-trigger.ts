...
export function unitDeath() {
	...
	TriggerAddCondition(
		...
		Condition(() => {
			...
			TransportManager.getInstance().onDeath(dyingUnit, killingUnit);
...
function enemySearch(guardChoice: unit, city: City, kUnit: unit, dUnit: unit): unit {
	...
	if (IsUnitType(guardChoice, UTYPE.ARTILLERY))
		guardChoice = CreateUnit(GetOwningPlayer(kUnit), UID.DUMMY_GUARD, city.x, city.y, 270);
	if (IsUnitType(guardChoice, UTYPE.GUARD))
		guardChoice = CreateUnit(GetOwningPlayer(kUnit), UID.DUMMY_GUARD, city.x, city.y, 270);
	if (IsUnitType(guardChoice, UTYPE.SHIP) && !city.isPort()) guardChoice = null;

	if (IsUnitType(kUnit, UTYPE.CITY))
		guardChoice = CreateUnit(GetOwningPlayer(dUnit), UID.DUMMY_GUARD, city.x, city.y, 270);
