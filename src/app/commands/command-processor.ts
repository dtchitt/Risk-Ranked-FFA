	...
	TriggerAddCondition(
		...
		Condition(() => {
			...
			switch (command) {
				...
				case '-def':
					CameraControls.getInstance().checkCamData(PlayerCamData.get(gPlayer.player), [
						I2S(CamSettings.DEFAULT_DISTANCE),
						I2S(CamSettings.DEFAULT_ANGLE),
				...
				case '-ff':
					...
					MessageAll(
						true,
						`${PLAYER_COLOR_CODES[gPlayer.names.colorIndex]}${gPlayer.names.acct}|r has ${
							HexColors.TANGERINE
						}forfeit|r the round!`
					);
				...
				case '-players':
					...
					lobbyTimer.start(0.75, true, () => {
						if (counter < names.length) {
							DisplayTimedTextToPlayer(gPlayer.player, 0.0, 0.0, 5, names[counter]);
						} else {
							lobbyTimer.pause();
							lobbyTimer.destroy();
				...
				case '-stfu':
					...
					stfuTimer.start(1, true, () => {
						if (duration < 1.0 || !GameTracking.getInstance().roundInProgress) {
				...
				case '-g':
					...
					try {
						...
						GamePlayer.fromPlayer.forEach((tPlayer) => {
							const compareName: string = RoundSettings.promode
								? tPlayer.names.acct.toLowerCase()
								: tPlayer.names.color.toLowerCase();
						...
						SetPlayerState(
							...
							GetPlayerState(gPlayer.player, PLAYER_STATE_RESOURCE_GOLD) - gQty
						);
						SetPlayerState(
							receiver.player,
							PLAYER_STATE_RESOURCE_GOLD,
							GetPlayerState(receiver.player, PLAYER_STATE_RESOURCE_GOLD) + gQty
						);
						DisplayTimedTextToPlayer(
							...
							0,
							3,
							`You sent ${HexColors.TANGERINE}${gQty}|r gold to ${receiver.coloredName()}`
						);
						DisplayTimedTextToPlayer(
							receiver.player,
							0,
							0,
							3,
							`You received ${HexColors.TANGERINE}${gQty}|r gold from ${gPlayer.coloredName()}`
						);
					} catch (error) {
						print(error);
					}
